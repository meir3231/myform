...
export default async function TemplatesPage() {
  ...
  const { data: forms } = await supabase
    .from("forms")
    .select("id, name, page_count, is_reusable, archived_at, created_at")
    .order("created_at", { ascending: false });

  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-paper-text">תבניות</h1>
      ...
      {!forms || forms.length === 0 ? (
        <div className="card border-dashed p-12 text-center">
          ...
          <p className="mb-4 text-paper-muted">עדיין אין תבניות. העלה PDF כדי להתחיל.</p>
              ...
              <div className="mb-3 flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {form.archived_at ? (
                      <span className="badge shrink-0 bg-slate-100 text-slate-500">הושבתה</span>
                    ) : form.is_reusable ? (
                      <span className="badge shrink-0 bg-brand/10 text-brand">שימוש חוזר</span>
                    ) : (
                      <span className="badge shrink-0 bg-amber-100 text-amber-700">חד-פעמי</span>
                    )}
                  </div>
