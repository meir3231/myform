...
export default async function SettingsPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="mb-6 text-2xl font-bold text-ink-text">הגדרות</h1>

      <div className="space-y-4">
        <section className="card-dark p-5">
          <h2 className="mb-4 font-semibold text-ink-text">פרטי משתמש</h2>
        ...
        <section className="card-dark p-5">
          <h2 className="mb-4 font-semibold text-ink-text">פרטי הארגון</h2>
          <dl className="space-y-3 text-sm">
...
function Row({ label, value, ltr }: { label: string; value: string; ltr?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-4 border-b border-ink-line pb-3 last:border-0 last:pb-0">
      ...
      <dd className="text-ink-text" dir={ltr ? "ltr" : undefined}>
        {value}
      </dd>
    </div>
  );
